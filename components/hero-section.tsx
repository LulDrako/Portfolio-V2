...
export default function HeroSection() {
  ...
  const y = useTransform(
    scrollY,
    [0, scrollHeight || 1],
    [70, windowHeight ? windowHeight - 220 : 500],
  );
  ...
  return (
    <section ref={containerRef} id="hero" className="relative min-h-screen flex items-center">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={`h-${i}`} className="hud-line" style={{ top: `${i * 20}%` }}></div>
        ))}
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={`v-${i}`} className="hud-line-vertical" style={{ left: `${i * 20}%` }}></div>
        ))}
      </div>

      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-[100vw] relative z-10">
        <motion.div
      ...
      >
        <Plane size={38} className="text-primary drop-shadow-lg" />
      </motion.div>
    </section>
  );
}
